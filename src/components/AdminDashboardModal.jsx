...
export default function AdminDashboardModal({ isOpen, onClose, currentUser }) {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/70 backdrop-blur-md p-4 animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden border border-gray-100 dark:border-slate-800 flex flex-col text-gray-800 dark:text-slate-100">
        ...
        <div className="p-6 border-b border-gray-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2.5 py-0.5 bg-red-600 text-white text-[10px] font-extrabold rounded-full uppercase">
          ...
          <button
            onClick={onClose}
            className="p-2 hover:bg-red-50 dark:hover:bg-red-950/40 rounded-2xl text-gray-400 hover:text-red-500 transition-colors cursor-pointer"
          >
        ...
        {/* Tab Navigation */}
        <div className="flex border-b border-gray-100 dark:border-slate-800 px-6 bg-gray-50/50 dark:bg-slate-900/30 gap-2 pt-2">
          <button
            onClick={() => setActiveTab('security')}
            className={`px-5 py-3 font-extrabold text-xs rounded-t-2xl transition-all cursor-pointer flex items-center gap-2 ${
              activeTab === 'security'
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
          ...
          {activeTab === 'security' && (
            <div className="space-y-6">
              ...
              {/* Global Lock Card */}
              <div className={`p-5 rounded-3xl border flex items-start justify-between gap-4 transition-all ${
                globalLock
                  ? 'bg-red-500/10 border-red-500/40 text-red-900 dark:text-red-200'
                  : 'bg-gray-50 dark:bg-slate-800/60 border-gray-200 dark:border-slate-700/60'
              }`}>
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className={`px-2 py-0.5 text-[10px] font-black rounded-md ${globalLock ? 'bg-red-600 text-white' : 'bg-gray-200 dark:bg-slate-700 text-gray-700 dark:text-slate-300'}`}>
                ...
                <label className="relative inline-flex items-center cursor-pointer shrink-0 mt-1">
                  ...
                  />
                  <div className="w-14 h-7 bg-gray-300 peer-focus:outline-none rounded-full peer dark:bg-slate-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-6 after:w-6 after:transition-all peer-checked:bg-red-600"></div>
                </label>
              </div>
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  className="w-full p-4 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl text-sm font-semibold text-gray-800 dark:text-slate-100 outline-none focus:ring-2 focus:ring-red-500"
                />
              </div>

              {/* Require Paid Access Switch */}
              <div className="p-5 rounded-3xl border border-gray-200 dark:border-slate-700/60 bg-gray-50 dark:bg-slate-800/60 flex items-start justify-between gap-4">
                <div className="space-y-1">
                  <h3 className="text-base font-extrabold text-gray-900 dark:text-white">
              ...
              <div className="flex justify-end pt-2">
                <button
                  ...
                  disabled={loading}
                  className="px-6 py-3 bg-red-600 hover:bg-red-700 text-white font-extrabold text-sm rounded-xl transition-all shadow-lg cursor-pointer"
                >
                  {loading ? '저장 중...' : '보안 설정 저장하기'}
                </button>
              </div>
          ...
          {activeTab === 'users' && (
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 bg-gray-50 dark:bg-slate-800/50 p-3 rounded-2xl border border-gray-100 dark:border-slate-800">
                <input
                  type="text"
                  placeholder="이메일 또는 UID 검색..."
                  value={userSearchTerm}
                  onChange={(e) => setUserSearchTerm(e.target.value)}
                  className="px-4 py-2 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl text-xs font-semibold outline-none focus:ring-2 focus:ring-blue-500 min-w-[240px]"
                />

                <div className="flex items-center gap-1.5 text-xs font-bold">
                  <button
                    onClick={() => setUserFilter('all')}
                    className={`px-3 py-1.5 rounded-lg transition-all ${userFilter === 'all' ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-slate-700 text-gray-700 dark:text-slate-300'}`}
                  >
                    전체 ({usersList.length})
                  </button>
                  <button
                    onClick={() => setUserFilter('paid')}
                    className={`px-3 py-1.5 rounded-lg transition-all ${userFilter === 'paid' ? 'bg-emerald-600 text-white' : 'bg-gray-200 dark:bg-slate-700 text-gray-700 dark:text-slate-300'}`}
                  >
                    유료 승인 ({usersList.filter(u => u.isPaid).length})
                  </button>
                  <button
                    onClick={() => setUserFilter('unpaid')}
                    className={`px-3 py-1.5 rounded-lg transition-all ${userFilter === 'unpaid' ? 'bg-amber-600 text-white' : 'bg-gray-200 dark:bg-slate-700 text-gray-700 dark:text-slate-300'}`}
                  >
                    미승인 ({usersList.filter(u => !u.isPaid).length})
                  </button>
                  <button
                    onClick={() => setUserFilter('blocked')}
                    className={`px-3 py-1.5 rounded-lg transition-all ${userFilter === 'blocked' ? 'bg-red-600 text-white' : 'bg-gray-200 dark:bg-slate-700 text-gray-700 dark:text-slate-300'}`}
                  >
              ...
              {/* Users Table */}
              <div className="border border-gray-100 dark:border-slate-800 rounded-2xl overflow-hidden shadow-2xs">
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs">
                    <thead className="bg-gray-100 dark:bg-slate-800/80 text-gray-600 dark:text-slate-400 font-extrabold uppercase tracking-wider border-b border-gray-200 dark:border-slate-700">
                      <tr>
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-slate-800 font-medium">
                      {filteredUsers.length === 0 ? (
                        ...
                        filteredUsers.map((u) => (
                          <tr key={u.uid} className="hover:bg-gray-50/80 dark:hover:bg-slate-800/50 transition-colors">
                            <td className="px-4 py-3 font-extrabold text-gray-900 dark:text-white">
                              {u.email}
                              {u.email === 'pc5@gmail.com' && (
                                <span className="ml-1.5 px-1.5 py-0.5 bg-red-600 text-white text-[9px] font-black rounded">
                                  ROOT
                                </span>
                              )}
                            </td>
                            <td className="px-4 py-3">
                              <span className={`px-2 py-0.5 rounded-full font-bold text-[10px] ${
                                u.role === 'admin' ? 'bg-purple-100 dark:bg-purple-950/60 text-purple-600 dark:text-purple-300' : 'bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-400'
                              }`}>
                            ...
                            <td className="px-4 py-3">
                              <span className={`px-2 py-0.5 rounded-full font-bold text-[10px] ${
                                u.isPaid ? 'bg-emerald-100 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-300' : 'bg-amber-100 dark:bg-amber-950/60 text-amber-600 dark:text-amber-300'
                              }`}>
                            ...
                            <td className="px-4 py-3">
                              <span className={`px-2 py-0.5 rounded-full font-bold text-[10px] ${
                                u.status === 'blocked' ? 'bg-red-100 dark:bg-red-950/60 text-red-600 dark:text-red-300' : 'bg-blue-100 dark:bg-blue-950/60 text-blue-600 dark:text-blue-300'
                              }`}>
                              ...
                              <div className="flex items-center justify-end gap-1.5">
                                <button
                                  ...
                                  className={`px-2.5 py-1 rounded-lg font-bold text-[10px] transition-all cursor-pointer ${
                                    u.isPaid ? 'bg-amber-100 hover:bg-amber-200 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300' : 'bg-emerald-600 hover:bg-emerald-700 text-white'
                                  }`}
                                >
                                ...
                                <button
                                  ...
                                  className={`px-2.5 py-1 rounded-lg font-bold text-[10px] transition-all cursor-pointer ${
                                    u.status === 'blocked' ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'bg-red-100 hover:bg-red-200 text-red-700 dark:bg-red-900/40 dark:text-red-300'
                                  }`}
                                >
            ...
            <div className="space-y-5">
              ...
              <div className="p-4 rounded-2xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/50 flex items-center justify-between">
                ...
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    ...
                    className="sr-only peer"
                  />
                  <div className="w-12 h-6 bg-gray-300 peer-focus:outline-none rounded-full peer dark:bg-slate-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-amber-600"></div>
                </label>
              </div>
              ...
              <div className="space-y-4">
                    ...
                    {['info', 'warning', 'alert'].map((t) => (
                      <button
                        ...
                        className={`flex-1 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                          annType === t
                            ? (t === 'alert' ? 'bg-red-600 text-white' : t === 'warning' ? 'bg-amber-600 text-white' : 'bg-blue-600 text-white')
                            : 'bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-400'
                        }`}
                      >
                        {t === 'info' ? '일반 공지' : t === 'warning' ? '주요 안내' : '긴급 공지'}
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl text-sm font-semibold outline-none focus:ring-2 focus:ring-amber-500"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full p-4 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl text-sm font-semibold outline-none focus:ring-2 focus:ring-amber-500"
                  />
                </div>
              </div>
              ...
              <div className="flex justify-end pt-2">
                <button
                  ...
                  disabled={loading}
                  className="px-6 py-3 bg-amber-600 hover:bg-amber-700 text-white font-extrabold text-sm rounded-xl transition-all shadow-lg cursor-pointer"
                >
                  {loading ? '저장 중...' : '공지사항 저장 및 즉시 발송'}
                </button>
              </div>
